...
import "../libs/openzeppelin-contracts/contracts/utils/Pausable.sol";
import "../libs/openzeppelin-contracts/contracts/utils/EnumerableMap.sol";
...
contract ClockAuctionBase is Ownable {
        ...
        returns (uint256)
    {
        ...
        if (price > 0) {
            ...
            uint256 dividentEth = auctioneerCut.div(dividentScale);
            IWETH(WETH).deposit{value: dividentEth}();
            for (uint256 i = 0; i < pairIndexList.length; i++) {
                uint256 pairIndex = pairIndexList[i];
                uint256 points = pairAllocPointMap[pairIndex];
                stakingMiningPool.supplyMinedToken(pairIndex, dividentEth.mul(points).div(totalAllocPoint));
            }            
        }
...
contract ClockAuction is Pausable, ClockAuctionBase {
    ...
    ///  between 0-10,000.
    constructor(address _nftAddress, uint256 _cut, address _WETH, address _stakingMiningPool) 
        ClockAuctionBase(_WETH, _stakingMiningPool) public {
        require(_cut <= 10000);
        ownerCut = _cut;
